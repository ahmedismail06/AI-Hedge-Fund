...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-surface text-on-surface selection:bg-primary-fixed selection:text-on-primary-fixed">
      <Sidebar />
      <div className={`${collapsed ? 'ml-[60px]' : 'ml-[220px]'} flex flex-col min-h-screen transition-all duration-200`}>
        {/* Macro regime strip */}
        {regime && (
          <div className={`flex items-center gap-3 px-6 py-1.5 border-b text-[11px] font-bold ${styles.bg}`}>
            ...
            <span className={styles.text}>{regime.regime}</span>
            ...
            {lastFetched && (
              <>
                <span className="text-gray-400 font-normal ml-auto">Updated {fmtAgo(lastFetched)}</span>
              </>
            )}
          </div>
        )}
