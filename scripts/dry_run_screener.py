...
def main():
    ...
    for i, cand in enumerate(sample_cands):
        ...
        _Q_KEYS = {"gross_margin", "revenue_growth_yoy", "roic", "fcf_conversion", "debt_to_equity", "eps_beat_rate"}
    ...
    if qualified:
        ...
        for r in sorted(qualified, key=lambda x: x.composite_score, reverse=True):
            ...
            print(f"    Raw quality:  gross_margin={_fmt_opt(q_rv.get('gross_margin'))}  "
                  ...
                  f"roic={_fmt_opt(q_rv.get('roic'))}  "
                  f"fcf_conv={_fmt_opt(q_rv.get('fcf_conversion'))}  "
