...
def print_diagnostics() -> None:
    ...
    # Quality raw values
    print(f"\n  Quality raw values:")
    print(f"    {'Ticker':<6}  {'GrossMargin':>12}  {'RevGrowth':>10}  {'ROIC':>8}  {'FCFConv':>8}  {'D/E':>8}  {'EPSBeat':>8}")
    for ticker in TICKERS:
        ...
        print(f"    {ticker:<6}  {_f(rv.get('gross_margin')):>12}  {_f(rv.get('revenue_growth_yoy')):>10}  "
              f"{_f(rv.get('roic')):>8}  {_f(rv.get('fcf_conversion')):>8}  {_f(rv.get('debt_to_equity')):>8}  {_f(rv.get('eps_beat_rate')):>8}")
